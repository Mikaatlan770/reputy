...
export class MockAiProvider implements AiProvider {
  ...
  private getPositiveTemplates(tone: AiTone, healthMode: boolean): string[] {
    const toneTexts: Record<string, string> = healthMode
      ? {
    ...
    const fallback = healthMode ? 'Merci pour votre confiance !' : 'Merci pour votre avis !'
    const genericTemplates = healthMode
      ? [
